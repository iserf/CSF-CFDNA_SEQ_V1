...
#RAW BAM#
#create mpileup
docker run --rm -v ${home_dir}:${home_dir} -u $UID --memory 200G 31071993/CSF_CFDNA_SEQ:special_positions /bin/bash -c \
"bcftools mpileup \
...
#Funcotation
docker run --rm -v ${home_dir}:${home_dir} -u $UID --memory 200G 31071993/CSF_CFDNA_SEQ:special_positions \
gatk Funcotator \
