...
date | tee -a $LOG_FILE
$pipeline_DIR/classify_variants.sh $ID_T $ID_N $BAM_T $home_dir $classifier $BED 2>>$ERROR_FILE 1>>$LOG_FILE
wait

echo "" | tee -a $LOG_FILE
echo "gatk Funcotator: SSeq.Classified.sSNV.pass.vcf and SSeq.Classified.sINDEL.pass.vcf" | tee -a $LOG_FILE
date | tee -a $LOG_FILE
$pipeline_DIR/funcotation.sh $ID_T $home_dir $BED 2>>$ERROR_FILE 1>>$LOG_FILE
wait
